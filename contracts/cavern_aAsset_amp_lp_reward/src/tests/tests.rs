...
use crate::{instantiate, execute, query, AmpLPRetrieve, RetrieveConfigRaw};
...
use super::mock_querier::{MOCK_ASTROPORT_PAIR, MOCK_AMP_LP_TOKEN, MOCK_AMP_HUB};

const DEFAULT_REWARD_DENOM: &str = "uusd";

fn default_init() -> InstantiateMsg<AmpLPRetrieve> {
    InstantiateMsg {
        ...
        retrieve_config: RetrieveConfigRaw { 
            amp_lp_token: MOCK_AMP_LP_TOKEN.to_string(),
            hub: MOCK_AMP_HUB.to_string(),
...
pub fn swap_to_reward_denom() {
    ...
    // We add a balance for the amp_lp_token and the astroport_lp_token

    deps.querier.with_token_balances(&[
        (&MOCK_AMP_LP_TOKEN.to_string(),&[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(5000000u128))]),
        (&MOCK_ASTROPORT_LP_TOKEN.to_string(), &[(&MOCK_CONTRACT_ADDR.to_string(), &Uint128::from(100u128))]),
    ]);
    ...
    assert_eq!(
        ...
        vec![
            SubMsg::new(
                CosmosMsg::Wasm(cosmwasm_std::WasmMsg::Execute { contract_addr: MOCK_AMP_LP_TOKEN.to_string(), msg: to_binary(&Cw20ExecuteMsg::Send{
                    contract: MOCK_AMP_HUB.to_string(),
                    amount: 5000000u128.into(),
                    msg: to_binary(&basset::eris_lp::Cw20HookMsg::Unbond{receiver: None}).unwrap()
                }).unwrap(), funds: vec![] })
            ),
            SubMsg::reply_on_success( 
                CosmosMsg::Wasm(cosmwasm_std::WasmMsg::Execute {
                    ...
                    msg: to_binary(&Cw20ExecuteMsg::Send {
                        amount: (5000000u128/4u128 + 100u128).into(),
                        contract: MOCK_ASTROPORT_PAIR.to_string(),
                        msg: to_binary(&astroport::pair::Cw20HookMsg::WithdrawLiquidity { assets: vec![] }).unwrap(),
                }).unwrap(),
            funds: vec![],
